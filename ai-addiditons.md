...
● Everything looks good. Both changes are correct. Here's a summary of what was done:

  CesiumMap.jsx:
  - The return block now has an outer aqo-map-shell (preserves the left: 60px sidebar offset) wrapping an aqo-map-wrapper flex column
  - Canvas div (aqo-map-canvas) is 70vh normally or 100vh in background mode — the loading overlay lives inside it so it only covers the canvas
  - All panels (tour controls, stats, environment, legend) render in aqo-map-ui-section below the canvas, hidden when backgroundMode is true
  - Tour popups stay as position: fixed overlays so they float at the bottom without permanently blocking the map

  globals.css:
  - Added aqo-map-wrapper (flex column), aqo-map-canvas (relative, explicit height), aqo-map-ui-section (flex row with wrapping)
  - .aqo-cesium-container changed from height: 100vh → height: 100% so it fills its canvas parent
  - Removed position: absolute + coordinates from .aqo-map-stats-panel, .aqo-map-legend, .aqo-environment-overlay, and .aqo-tour-controls
  - Tour popups changed to position: fixed; bottom: 2rem / 7rem with pointer-events: none
  - Media query at ≤900px now sets aqo-map-canvas to 60vh and stacks the UI section vertically instead of fighting with absolute positions

  alot of the points are missplaced 
...
const CesiumMap = ({ 
  ...
  const tourWaypoints = [
    ...
    { lon: -75.15575350419238, lat: 40.01968402262896, targetHeight: 18, range: 150, heading: 46, pitch: -28, duration: 4, wait: 1800, name: 'Roosevelt Extension pollution corridor' },
  ...
  const applyCinematicState = (viewer, Cesium, enabled) => {
    ...
    viewer.scene.fog.density = enabled ? 0.00045 : 0.00018;
    viewer.scene.fog.minimumBrightness = enabled ? 0.18 : 0.42;
    viewer.scene.highDynamicRange = enabled;
    viewer.scene.skyAtmosphere.show = true;
    viewer.scene.globe.atmosphereBrightnessShift = enabled ? -0.04 : 0.01;
    viewer.scene.globe.atmosphereHueShift = enabled ? -0.01 : 0;
  ...
  //==============================================================
  // This function is used to fly the camera to a specific location on the map when a marker is clicked, it takes the viewer instance, the Cesium library reference, and the entity (marker) that was clicked as parameters, it checks if the entity has a valid position and then uses Cesium's flyToBoundingSphere method to smoothly transition the camera to focus on that location with a specific offset for better viewing angle, this enhances user interactivity by allowing them to click on markers and have the map automatically navigate to those points of interest for a closer look.
  //==============================================================

  const flyToSelectedLocation = (viewer, Cesium, entity) => {
    if (!entity?.position) {
      return;
    }
    ...
    setCameraMode('free');
    setActiveSceneLabel(entity.name || 'Selected location');
    setTourDebug(`focused on ${entity.name || 'selected marker'}`);

    viewer.camera.flyToBoundingSphere(new Cesium.BoundingSphere(entityPosition, 55), {
  ...
  useEffect(() => {
    ...
    const loadCesium = async () => {
      try {
        ...
        const useGooglePhotorealisticTiles = process.env.NEXT_PUBLIC_USE_GOOGLE_3D === 'true';
        if (googleMapsKey) {
        ...
        try {
          ...
          console.warn('Failed to load Cesium World Terrain. Falling back to default ellipsoid.', terrainError);
        }
        ...
        const viewer = new Cesium.Viewer(containerRef.current, {
          ...
          geocoder: useGooglePhotorealisticTiles ? Cesium.IonGeocodeProviderType.GOOGLE : false,
        ...
        if (!backgroundMode) {
          viewer.selectedEntityChanged.addEventListener((selectedEntity) => {
            ...
            flyToSelectedLocation(viewer, Cesium, selectedEntity);
          });
        }
        ...
        pollutionSources.forEach(source => {
          ...
          viewer.entities.add({
            ...
            position: Cesium.Cartesian3.fromDegrees(source.lon, source.lat),
            ellipse: {
              semiMajorAxis: new Cesium.CallbackProperty(() => 24 + Math.abs(Math.sin(Date.now() / 420)) * 58, false),
              semiMinorAxis: new Cesium.CallbackProperty(() => 24 + Math.abs(Math.sin(Date.now() / 420)) * 58, false),
              material: Cesium.Color.fromCssColorString(source.alertColor).withAlpha(0.08),
              outline: true,
              outlineColor: Cesium.Color.fromCssColorString(source.alertColor).withAlpha(0.42),
              height: 2
            }
          });
          ...
          for (let i = 0; i < 10; i += 1) {
            ...
            viewer.entities.add({
              ...
              point: {
                pixelSize: 3,
                color: Cesium.Color.fromCssColorString(source.alertColor).withAlpha(0.24),
        ...
        communitySolutions.forEach(solution => {
          ...
          viewer.entities.add({
            ...
            position: Cesium.Cartesian3.fromDegrees(solution.lon, solution.lat),
            ellipse: {
              semiMajorAxis: new Cesium.CallbackProperty(() => 18 + Math.abs(Math.sin(Date.now() / 560)) * 38, false),
              semiMinorAxis: new Cesium.CallbackProperty(() => 18 + Math.abs(Math.sin(Date.now() / 560)) * 38, false),
              material: Cesium.Color.fromCssColorString('#4CAF50').withAlpha(0.05),
              outline: true,
              outlineColor: Cesium.Color.fromCssColorString('#7ef0a0').withAlpha(0.34),
              height: 1
            }
          });
        });
        ...
        const createWindFlow = (startLon, startLat, endLon, endLat, color) => {
          for (let i = 0; i < 8; i += 1) {
            viewer.entities.add({
              ...
              point: {
                pixelSize: 3,
                color: Cesium.Color.fromCssColorString(color).withAlpha(0.42)
              }
            });
          }
        ...
        let hasPhotorealisticContext = false;

        if (useGooglePhotorealisticTiles) {
          try {
            ...
            hasPhotorealisticContext = true;
          } catch (error) {
            console.warn('Failed to load Google Photorealistic 3D Tiles. Falling back to OSM buildings.', error);
          }
        }

        // Fallback 3D buildings from OpenStreetMap data when photorealistic tiles are unavailable.
        try {
          if (!hasPhotorealisticContext) {
            const tileset = await Cesium.createOsmBuildingsAsync({
              style: new Cesium.Cesium3DTileStyle({
                color: {
                  conditions: [
                    ["Number(${feature['cesium#estimatedHeight']}) >= 80", "color('#7a838d', 0.96)"],
                    ["Number(${feature['cesium#estimatedHeight']}) >= 40", "color('#8f98a1', 0.95)"],
                    ["Number(${feature['cesium#estimatedHeight']}) >= 15", "color('#a7b0b7', 0.94)"],
                    ['true', "color('#c1c8ce', 0.92)"]
                  ]
                }
              })
            });
        ...
        // Use hand-authored building masses only when the real photorealistic city mesh is unavailable.
        if (!hasPhotorealisticContext) {
          viewer.entities.add({
            ...
            polygon: {
              ...
              extrudedHeight: 15.0,
              material: Cesium.Color.fromCssColorString('#FF6B35').withAlpha(0.45),
              outline: true,
        ...
        viewer.entities.add({
          ...
          polygon: {
            ...
            height: 0,
            material: Cesium.Color.fromCssColorString('#FF6B35').withAlpha(hasPhotorealisticContext ? 0.12 : 0.3),
            outline: !hasPhotorealisticContext,
            outlineColor: Cesium.Color.fromCssColorString('#ffd7c5').withAlpha(0.45)
          },
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: backgroundMode ? '100%' : '600px', borderRadius: backgroundMode ? '0' : '12px', overflow: 'hidden', pointerEvents: backgroundMode ? 'none' : 'auto' }}>
      ...
      <div ref={containerRef} style={{ position: 'relative', zIndex: 1, width: '100%', height: '100%', filter: backgroundMode ? 'saturate(0.82) brightness(0.7) contrast(1.08)' : 'none' }} />
      ...
      {!isLoaded && !error && (
        <div style={{
          ...
          zIndex: 1000  // Appears above the map div
        }}>
